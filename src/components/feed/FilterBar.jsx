...
export default function FilterBar({
  ...
  return (
    <section className="mb-6 sm:mb-10 bg-paperDark dark:bg-nightCard p-3.5 sm:p-6 md:p-7 rounded-xl border border-gray-200/80 dark:border-slate-700/80 shadow-inner transition-colors duration-200 space-y-4 sm:space-y-5">
      {/* Row 1: Issue & Category Selectors + Action Buttons */}
      <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-end gap-4 sm:gap-6">
        <div className="flex flex-col sm:flex-row gap-3.5 sm:gap-6 w-full lg:w-auto flex-1">
        ...
        <div className="grid grid-cols-2 sm:flex sm:flex-row gap-2.5 sm:gap-3 w-full lg:w-auto shrink-0">
          <button
            type="button"
            onClick={onOpenLatexModal}
            className="w-full sm:w-auto bg-white dark:bg-night border border-cerulean dark:border-blue-400 text-cerulean dark:text-blue-400 font-playfair font-bold text-sm sm:text-base px-3 sm:px-4 py-2.5 md:py-3 rounded-lg hover:bg-blue-50 dark:hover:bg-slate-800 transition shadow-xs flex items-center justify-center gap-1.5 sm:gap-2 shrink-0 cursor-pointer min-h-[44px]"
            title="Mở danh mục tra cứu tất cả công thức toán LaTeX"
          >
            <svg className="w-4 h-4 sm:w-5 sm:h-5 text-cerulean dark:text-blue-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
            </svg>
            <span className="truncate">Sổ Tay LaTeX</span>
          </button>

          <button
            type="button"
            onClick={onOpenProblemModal}
            className="w-full sm:w-auto bg-ink dark:bg-cerulean text-white font-playfair font-bold text-sm sm:text-base md:text-lg px-3 sm:px-7 py-2.5 md:py-3 rounded-lg hover:bg-gray-800 dark:hover:bg-blue-800 transition shadow-md flex items-center justify-center gap-1.5 sm:gap-2 shrink-0 cursor-pointer min-h-[44px]"
          >
            <svg className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
            </svg>
            <span className="truncate">Soạn Đề Mới</span>
          </button>
        </div>
      </div>

      {/* Row 2: Search Bar with '/' Shortcut + Advanced Filter Toggle */}
      <div className="pt-2 border-t border-gray-200/70 dark:border-slate-800/80 flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-3 font-newsreader">
        {/* Search Input Box */}
        <div className="relative flex-1">
          <input
            ...
            placeholder="Tìm mã (P1...), đề, tác giả... (Nhấn /)"
            className="w-full pl-9 pr-10 sm:pr-14 py-2.5 sm:py-2 bg-white dark:bg-nightInput border border-gray-300 dark:border-slate-700 rounded-lg text-sm text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-cerulean placeholder-gray-400 dark:placeholder-slate-500 font-newsreader shadow-2xs min-h-[44px] sm:min-h-0"
          />
          {/* Search SVG icon */}
          <svg className="w-4 h-4 text-gray-400 dark:text-slate-500 absolute left-3 top-3.5 sm:top-2.5 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {filterSearch ? (
            <button
              ...
              className="absolute right-2.5 top-2.5 sm:top-2 p-1 text-gray-400 hover:text-jasper dark:hover:text-rose-400 rounded-md transition cursor-pointer"
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <button
            ...
            className={`flex-1 sm:flex-initial flex items-center justify-center gap-2 px-3.5 py-2.5 sm:py-2 rounded-lg border text-sm font-bold transition cursor-pointer font-newsreader shadow-2xs min-h-[44px] sm:min-h-0 ${
          ...
          {hasAnyFilterActive && (
            <button
              ...
              className="flex items-center justify-center gap-1.5 px-3 py-2.5 sm:py-2 text-xs font-bold text-jasper dark:text-rose-400 bg-red-50 dark:bg-rose-950/40 border border-jasper/30 dark:border-rose-900/60 rounded-lg hover:bg-red-100/70 dark:hover:bg-rose-900/40 transition cursor-pointer font-newsreader shadow-2xs whitespace-nowrap min-h-[44px] sm:min-h-0"
