...
export default function CategoryModal({ isOpen, onClose, onRequestDelete }) {
  ...
  return (
    <div className="fixed inset-0 bg-ink/60 z-[100] flex justify-center items-center backdrop-blur-sm px-4">
      <div className="bg-paper p-6 md:p-8 rounded-xl shadow-2xl w-full max-w-lg border-t-4 border-jasper max-h-[90vh] flex flex-col animate-dropdownFade">
        <div className="flex justify-between items-center mb-5 pb-3 border-b border-gray-200">
          <div>
            <h3 className="font-playfair text-2xl font-bold text-ink">Quản Lý Chuyên Mục</h3>
            <p className="text-xs text-gray-500 font-newsreader mt-0.5">Thêm hoặc xóa các chuyên mục bài toán của tạp chí</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="mb-6 bg-white p-4 rounded-xl border border-gray-200 shadow-2xs">
          <label className="block text-xs font-bold text-jasper font-playfair uppercase tracking-wider mb-1.5">
            Thêm Chuyên Mục Mới
          </label>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 border border-gray-300 bg-paper rounded-lg px-3.5 py-2 font-newsreader text-base focus:outline-none focus:ring-2 focus:ring-jasper transition"
            />
            <button
              type="submit"
              disabled={isSubmitting}
              className="bg-jasper text-white px-5 py-2 rounded-lg hover:bg-red-800 transition shadow-xs font-newsreader font-bold text-sm tracking-wide cursor-pointer shrink-0 disabled:opacity-50"
            >
              Thêm
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto pr-1">
          ...
          <ul className="divide-y divide-gray-100 bg-white rounded-xl border border-gray-200 overflow-hidden shadow-2xs font-newsreader">
            {categories.map((c) => (
              <li key={c.id} className="py-3 px-3.5 flex justify-between items-center group hover:bg-gray-50 transition">
                <div className="flex items-center gap-2.5 min-w-0">
                  <span className="w-2.5 h-2.5 rounded-full bg-jasper shrink-0"></span>
                  <span className="font-newsreader font-bold text-base text-ink truncate">{c.name}</span>
                </div>
                <button
                  type="button"
                  onClick={() =>
